...
/**
 * Load trips from database asynchronously in a background thread.
...
public class ActivitiesLoader extends AsyncTaskLoader<List<UserActivity>> {
    ...
    // The observer to notify the Loader when there is a new trip.
    private NewActivityReceiver mNewActivityReceiver;

    private final String TAG = "TripLoader";
    ...
    public List<UserActivity> loadInBackground() {
        ...
        mUserActivities = ActivitiesDBManager.getInstance().getTrips();
    ...
    protected void onStartLoading() {
        ...
        if (takeContentChanged() || mUserActivities == null) {
            // When the new trip receiver receives a new trip broadcast, calls
    ...
    protected void onReset() {
        ...
        // The Loader is being reset, so we should stop listening for new trips broadcast.
        if (mNewActivityReceiver != null) {
